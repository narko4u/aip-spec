...
[![OpenSSF Best Practices - Baseline 1](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fwww.bestpractices.dev%2Fprojects%2F14143.json&query=badge_percentage_baseline_1&label=OpenSSF%20Baseline%201&suffix=%25&color=success)](https://www.bestpractices.dev/projects/14143) [![OpenSSF Best Practices - Baseline 2](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fwww.bestpractices.dev%2Fprojects%2F14143.json&query=badge_percentage_baseline_2&label=OpenSSF%20Baseline%202&suffix=%25&color=success)](https://www.bestpractices.dev/projects/14143) [![OpenSSF Best Practices - Baseline 3](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fwww.bestpractices.dev%2Fprojects%2F14143.json&query=badge_percentage_baseline_3&label=OpenSSF%20Baseline%203&suffix=%25&color=success)](https://www.bestpractices.dev/projects/14143)
...
1. **Selection** - dependencies are avoided unless a standard-library
   alternative does not exist. The reference implementation has zero
   external Go dependencies (stdlib only).
2. **Obtaining** - dependencies are declared in `go.mod`; builds are
   reproducible via Go modules and the checksums in `go.sum`.
3. **Tracking** - dependencies are monitored three ways:
   - **SCA**: every push/PR runs [OSV-Scanner](https://google.github.io/osv-scanner/)
     in the `security` workflow to detect known vulnerabilities.
   - **SBOM**: every release ships CycloneDX SBOMs (`*.cdx.json`) generated by
     goreleaser + syft, listing the exact dependency set of each archive.
   - **Integrity**: every release asset ships with a Sigstore signature
     bundle (`.sigstore.json`) and a signed `checksums.txt` (see
